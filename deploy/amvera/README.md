...
| [`../../amvera.yml`](../../amvera.yml) | конфиг Amvera **в КОРНЕ репо** (иначе не читается): `containerPort 80`, `persistenceMount /data`, путь к Dockerfile |

## Ограничения Amvera (важно)

- **Только HTTP(S) наружу** через nginx: домен → `servicePort` → `containerPort`. По умолчанию оба
  = 80, поэтому Ollama слушает 80 (`OLLAMA_HOST=0.0.0.0:80`) — проброс работает без возни с портами.
  Произвольные TCP нельзя, но REST Ollama (`/api/*`) — это HTTP.
- **Бесплатный домен не создаётся автоматически** — его надо добавить в панели (см. шаг 5), иначе
  обращение к URL отдаёт 404-страницу *самой Amvera* (edge не находит бэкенд).
...
4. Соберите и запустите. **Первый старт качает модель** (~1 ГБ) — это медленно; последующие быстрые
   (модель на `/data`). Дождитесь в логах запуска, что модель подтянулась и Ollama слушает.
5. **Привяжите домен** (обязательно, сам не появится): панель → **Настройки → Доменные имена →
   Добавить → «Бесплатный домен Амвера»**. Он привяжется к порту 80 (совпадает с `containerPort`).
   URL будет вида `https://<проект>.<user>.amvera.io` — берите точный из этого раздела.
