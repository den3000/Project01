...
class SlashCommandParserTest {
    ...
    fun `when branch bare - then ListBranches`() {
        // when - then
        assertEquals(SessionCommand.ListBranches, parseSlashCommand("/branch"))
    }

    @Test
    fun `when branch with a name - then Branch forks`() {
        // when - then
        assertEquals(SessionCommand.Branch("exp"), parseSlashCommand("/branch exp"))
    }

    @Test
    fun `when branch switch with a name - then Switch`() {
        // when - then
        assertEquals(SessionCommand.Switch("exp"), parseSlashCommand("/branch switch exp"))
    }

    @Test
    fun `when branch show - then Checkpoint`() {
        // when - then — `branch show` = current branch + message count
        assertEquals(SessionCommand.Checkpoint, parseSlashCommand("/branch show"))
    }
    //endregion
    ...
    fun `when memory - then ShowMemory`() {
        // when - then
        assertEquals(SessionCommand.ShowMemory, parseSlashCommand("/memory"))
    }
    ...
    fun `when profile clear bare - then ClearAllProfiles`() {
        ...
        assertEquals(SessionCommand.ClearAllProfiles, parseSlashCommand("/profile clear"))
    }

    @Test
    fun `when profile with a bare name - then SwitchProfile activates it`() {
        // when - then
        assertEquals(SessionCommand.SwitchProfile("work"), parseSlashCommand("/profile work"))
    }
    ...
    fun `when profile show with a name - then ShowProfile (verb-then-name)`() {
        ...
        assertEquals(SessionCommand.ShowProfile("work"), parseSlashCommand("/profile show work"))
    ...
    fun `when profile bare - then ListProfiles`() {
        // when - then
        assertEquals(SessionCommand.ListProfiles, parseSlashCommand("/profile"))
    }
    //endregion
    ...
    fun `when rule with quoted text - then AddRule`() {
        // when - then
        assertEquals(SessionCommand.AddRule("no emojis"), parseSlashCommand("/rule \"no emojis\""))
    }

    @Test
    fun `when rule clear with an id - then RemoveRule`() {
        // when - then
        assertEquals(SessionCommand.RemoveRule("003"), parseSlashCommand("/rule clear 003"))
    }

    @Test
    fun `when task with an id - then SetTask`() {
        // when - then
        assertEquals(SessionCommand.SetTask("fix-bug"), parseSlashCommand("/task fix-bug"))
    }

    @Test
    fun `when task pause - then PauseTask`() {
        // when - then
        assertEquals(SessionCommand.PauseTask, parseSlashCommand("/task pause"))
    }

    @Test
    fun `when agent mode system - then SetMemoryMode SYSTEM`() {
        // when - then
        assertEquals(SessionCommand.SetMemoryMode(MemoryMode.SYSTEM), parseSlashCommand("/agent mode system"))
    }
