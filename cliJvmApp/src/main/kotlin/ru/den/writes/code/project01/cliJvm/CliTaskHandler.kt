 ...
 * text for the engine to store; an agent task injects a turn (wired in the agent step) and
 * returns null. [actions] is filled after the tasks are added, so it is read at tick time.
 */
internal class CliTaskHandler(
    ...
    override suspend fun handle(task: ScheduledTask): String? = when (val action = actions[task.id]) {
        ...
        is ScheduleAction.Agent -> null // wired to submitFromScheduler in the agent step
        null -> null
    }
}
