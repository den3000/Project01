 ...
 * callers don't have to keep their own copy.
 *
 * Thin wrapper over [MessageDao] that hides Room types from the rest
 * of the codebase. Translation between the neutral [Message] / [Role]
 * (which cross the [HistoryStore] boundary) and the persisted
 * [MessageEntity] (string `role` column, `session_id` discriminator)
 * lives here.
 *
...
internal class RoomHistoryStore(
     ...
     */
    override var branchId: String = initialBranch
        private set
     ...
     * need a snapshot.
     */
    override val messages: List<Message> get() = cache
     ...
     * it.
     */
    override var stats: SessionStats = SessionStats()
        private set
     ...
     * [load].
     */
    override suspend fun loadSummary(): SummarySnapshot? =
        dao.getSummary(sessionId, branchId)?.let { SummarySnapshot(it.summaryText, it.coveredCount) }
     ...
     */
    override suspend fun saveFacts(factsJson: String, modelId: String?, usage: Usage?) {
    ...
    override suspend fun loadFacts(): FactsSnapshot? =
        dao.getFacts(sessionId, branchId)?.let { FactsSnapshot(it.factsJson) }
     ...
     */
    override suspend fun switchTo(newBranch: String) {
        branchId = newBranch
        load()
    }

    /** All branch ids that exist for this session, in order of first appearance. */
    override suspend fun branches(): List<String> = dao.branchesOf(sessionId)
     ...
     */
    override suspend fun fork(newBranch: String) {
     ...
     */
    override suspend fun deleteBranch(branch: String) {
