...
- `CliArgs.kt` — **legacy**-парсер (sealed `CliArgs`: ListSessions/Clean/Inflate/Chat/OneShot/Memory) + `USAGE` + `buildModelProvider` (internal). Теперь это парс-DTO, мост в домен — `CliArgsToCommand`; `MemoryAction` переехал в `command/`.
- **`command/` — доменный слой + парсеры** (расслойка «что/как»): sealed `CliCommand` (`RunChat`/`RunOneShot`/`ListSessions`/`CleanHistory`/`InflateSession`/`MemoryOp`, общий `RunPrompt`) — чистые данные, отвязаны от парсинга; `MemoryAction`. `CommandParser` (интерфейс `Array<String>→CliCommand` + `ApiKeys`) с двумя реализациями: `LegacyCommandParser` (`CliArgs.from`→`CliArgsToCommand`, **рантайм-путь main**) и `CliControlsCommandParser` (`CliControlsParser.parseArgv`→`ControlsToCommand`, полный паритет, **test-only**); `ParseErrorMapping` (`ParseError→CliArgsException`). `CommandExecutor.kt` (живёт в пакете `cliJvm`, рядом с рантаймом) исполняет `CliCommand`: list/clean/inflate/memory + chat/oneshot (HttpClient + MVI-стек) — извлечено из `main`.
