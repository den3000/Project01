...
- `CliArgsException.kt` — типизированные ошибки разбора (`MissingRequiredArgument`/`InvalidArgumentValue`/`TooManyValues`); единственное, что осталось от бывшего `CliArgs.kt` (legacy-парсер удалён). Их бросает новый фронт (`ParseErrorMapping`/`ControlsToCommand`/`ControlsToBranchCommand`); `main` печатает + `USAGE` на missing-required.
...
- **Stage-маркер `[[stage:...]]` не вырезается из ответа** (БАГ, чинить): `AgentResponder.respond` (`:shared`) парсит сигнал (`parseStageSignal`), но возвращает сырой `result.text` — маркер протекает и в показанный ответ, и в persist истории. Gemini stateless → история с маркером ре-шлётся каждый ход → модель видит свой же маркер и плодит новые (особенно если в правилах есть живой пример `[[stage:...]]` — он тоже инжектится в промпт). Невалидный keyword (`clarifying_requirements`) FSM игнорит (`byKeyword`→null, перехода нет) — это ок, но маркер всё равно виден из-за этого же. Фикс: срезать распознанный `[[stage:...]]` из reply перед persist/показом (в `TurnEngine`/`AgentResponder`). Найдено при ручном TUI-прогоне; не регрессия command-рефактора.
...
- **Единый каталог clicontrols питает оба фронта** (`-flag` startup и `/cmd` in-session) через `CliControls.all`; legacy `CliArgs.from` + ручной `parseSlashCommand`-when удалены. Спеллинги old→new: startup `-sessions`→`-session`, `-clean`→`-session clear`, `-memory <op>`→`-profile/-rule/-task <op>`, генерация-knobs + `-memory-mode`/`-stageAgent`/`-judgeAgent` → под `-agent` (`provider/model/maxTokens/temperature/stopSequence/endSequence/profile/mode/stages/judge`). In-session: `/checkpoint`→`/branch show`, `/branches`→`/branch`, `/switch x`→`/branch switch x`, `/profiles`→`/profile`, `/profile-use x`→`/profile x`, `/profile-show x`→`/profile show x`, `/memory-mode`→`/agent mode`, `/task-note|pause|resume`→`/task note|pause|resume`. Удаление везде единым `clear`: `<entity> clear <name>` = один, bare = все (rule/task/profile/session; rule-only `rm` выкинут). **Verb-then-name строго**: `<entity> <name> show|clear` — НЕ команда. Многословные значения — в кавычках. Free-text профиль (`profile <text>`→`SetProfile`) дропнут.
