...
- **Задача (FSM)** — задача двигается `clarification → planning → execution → validation → done`
  ...
  - **Дошла до `done` — снимается с сессии**: файл остаётся на диске, но задача перестаёт быть
    активной (`[task] '<id>' is done — no longer the active task`), стадия больше не инжектится,
    ходы после неё — обычный чат. То же и при старте, если в файле уже `done`.
  - **Ход, не сдвинувший задачу, платит** ретраем стадии (нет маркера, re-signal, запрещённый
    переход, ход по пройденной земле, судья снял ответ). Стадийный бюджет кончился — задача
    перезапускается с нуля на свежей ветке истории, задачный кончился — прекращается. И то, и
    другое видно в ленте строкой `[fsm]` (в TUI — колонка `fsm │`); правила и потолки —
    [features:fsm](../../features/fsm/README.md).
