...
- `di/`: `fileSystemModule` — Koin-модуль, биндит `LocalFileSystem` (`internal expect` + `public val`;
  jvm actual `single { JvmLocalFileSystem() }`, android/ios `TODO`). Рядом `fileSystemTestModule` (val) —
  `factory<LocalFileSystem> { LocalFileSystemFake() }` (фейк `LocalFileSystemFake` отдельным файлом,
  in-memory; свежий инстанс на `get()`; для integration-графов вместо `fileSystemModule`). Общая дока —
  [DI.md](../../DI.md).
