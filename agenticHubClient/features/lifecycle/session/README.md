...
`InlineFsmTurnEngine` (нудж, пауза, приватный счётчик + контракт хода). Правила задачи не
проверяются ни там, ни там: они в `features:fsm`.

**Задача, дошедшая до `done`, снимается с сессии** (`retireFinishedTask`): файл остаётся на диске,
активной она быть перестаёт — и по ходу, которым дошла, и при старте, если в файле уже `done`.
Иначе каждый следующий ход одет как работа над ней: task-слой в промпте с целью и `allowed-next`,
агент и судья выбираются по стадии, которая уже не двинется, а машину спрашивают про реплики,
которые просто разговор. Живой прогон показал, чем это кончается: безголовый прогон продолжал
подавать «продолжай», модель раз за разом сигналила `done`, стадийный бюджет исчерпался на этой
болтовне — и машина перезапустила успешно сданную задачу. Разговор при этом продолжается: сессия
переживает задачу, дальше идёт обычный чат.

**В бою стоит `FsmTurnEngine`** — его собирает `buildSessionViewModel`, машина приходит из графа
(`fsmModule`). `InlineFsmTurnEngine` остался второй ногой для сравнения: из реальной сессии в него
больше не приходят, а в тестах он выбирается через `EngineUnderTest` (`INLINE_ENGINE`/`FSM_ENGINE`).
Вердикт машины (`TurnResult.retryOutcome`) читает `SessionViewModel`: рестарт и сдача едут в
транскрипт строкой `[fsm]`, сдача вдобавок абортит feed-источник (stdin и TUI её игнорируют —
кончается задача, а не разговор), и объявляется один раз, потому что машина не меняет задачу,
на которой сдалась.
...
- **Тихий NO_MOVE-лок FSM на слабой модели** — модель шлёт маркер ТЕКУЩЕЙ стадии (`[[stage:validation]]`
  ...
  несущим механизмом. У `FsmTurnEngine` флага нет: нудж взведён от стадийного бюджета, который
  держит машина, поэтому разойтись с бюджетом, о котором предупреждает, он не может.
- **`/task pause` живёт снаружи автомата.** Флаг `paused` лежит в `TaskNotes`, а машина его
  намеренно не моделирует, поэтому честит его движок: ход на паузе доходит до модели и до истории,
  но до машины — нет (стадия держится, бюджеты целы). Асимметрия известная и не закрытая: пауза —
  единственное правило задачи, которое живёт не в `features:fsm`, и если правил станет два, решать
  придётся по-настоящему.
