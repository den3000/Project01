 ...
 * [UpdateReason]), half check that the answer is applied (the task is written down, a restart
...
class FsmTurnEngineTest {
    ...
    fun `when the reply names a stage - then the machine is asked about that stage`() = runTest {
        ...
        // then
        assertEquals(listOf<UpdateReason>(UpdateReason.StageProposed(Stage.EXECUTION)), machine.asked)
    }

    @Test
    fun `when the reply names no stage - then the machine is told nothing was proposed`() = runTest {
        // given
        val machine = RecordingMachine()
    ...
    @Test
    fun `when the judge withdraws the reply - then the machine is told the answer was blocked`() = runTest {
        // given
        val machine = RecordingMachine()
        val judge = RecordingJudge(
            InvariantVerdict(passed = false, violations = listOf(InvariantViolation("r1", "nope"))),
        )
        val api = scriptedApi(FakeLlmScript().apply { queueText("breach [[stage:execution]]"); queueText("breach again") })
    ...
    @Test
    fun `when the provider cannot be reached - then the machine is told the transport failed`() = runTest {
        // given
        val machine = RecordingMachine()
    ...
    fun `when there is no active task - then the machine is not asked at all`() = runTest {
        // given
        val machine = RecordingMachine()
        val api = scriptedApi(FakeLlmScript().apply { queueText("just talking [[stage:execution]]") })
    ...
    @Test
    fun `when the machine restarts the task - then the next turn talks on a fresh branch`() = runTest {
        // given
        val restarted = Task(taskId = TASK_ID, taskRetryState = RetryState(attempt = 1, max = RetryState.TASK_MAX))
        val machine = RecordingMachine { decision(restarted, retryOutcome = RetryOutcome.Restarted(restarted)) }
     ...
     * and returns whatever [answer] says. Default answer is "nothing happened", which is the
     * quietest thing a decision can be — the engine then has nothing to apply.
     */
    private class RecordingMachine(
        private val answer: (Task) -> UpdateDecision = { decision(it) },
    ) : TaskStateMachine {
    ...
    private fun task() = TaskNotes(taskId = TASK_ID, stage = TaskStage.EXECUTION, deepestStage = TaskStage.EXECUTION)
