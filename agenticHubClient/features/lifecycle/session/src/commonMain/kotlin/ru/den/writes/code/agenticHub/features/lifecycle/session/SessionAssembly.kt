...
public fun buildSessionViewModel(
    ...
    val scheduler = if (schedulerEnabled) {
        SchedulerEngine(InMemoryScheduleStore(), handler, now = { nowMillis() })
    } else {
        null
    }
...
/**
 * Launch the scheduler loops on the platform [schedulerDispatcher]: a ticker that adds the startup tasks
...
public fun CoroutineScope.startSchedulerLoops(
    ...
    val ticker = launch(schedulerDispatcher()) {
        for (spec in schedules) control.add(spec)
        engine.runLoop(SCHEDULER_TICK_MS)
    }
    val reporter = launch(schedulerDispatcher()) {
