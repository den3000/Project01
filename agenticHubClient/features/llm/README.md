...
- `di/`: `llmModule` — `factory<LlmApi> { (mp: ModelProvider) -> buildLlmApi(mp, get()) }` (`ModelProvider`
  runtime, `HttpClient` из графа). Рядом `llmTestModule` — биндит `LlmApi` на `internal ScriptedLlmApi`
  + публичный holder `FakeLlmScript` (скрипт ответов/инспекция вызовов, настраивается через
  `koin.get<FakeLlmScript>()`; для integration-графов вместо `llmModule`). Общая дока — [DI.md](../../DI.md).
