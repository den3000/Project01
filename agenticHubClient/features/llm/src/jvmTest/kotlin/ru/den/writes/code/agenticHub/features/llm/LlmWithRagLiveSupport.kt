 ...
 * coroutine test under the shared live timeout ([runLiveTest]), but skip it (JUnit
 * [assumeTrue]) when the local Ollama at [OLLAMA_BASE] isn't reachable. EVERY llm live
 * test needs it — either for generation (`LocalOllamaApiLiveTest`) or for embeddings
 * during retrieval (the RAG answer tests always embed through Ollama, even when the
