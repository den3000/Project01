...
sealed interface RetryOutcome {
    ...
    /**
     * The reason belongs to a level the task does not account for
     * ([RetryLevel.TURN]) — nothing was spent and nothing changed.
     */
    data class Untouched(override val task: Task) : RetryOutcome

    /** Same stage, one stage attempt lighter. Nothing else to do. */
