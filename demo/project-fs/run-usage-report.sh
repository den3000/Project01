...
#   PROVIDER=ollama MODEL=<тег> bash demo/project-fs/run-usage-report.sh   # локальная модель
#
set -euo pipefail
...
run_assistant \
  ...
  -task usage-report \
  -rag "$RAG_NAME" \
  -agent $PROVIDER_ARG $FALLBACK_MODEL_ARG $TEMP_ARG mode system \
  -agent fs-explorer $PROVIDER_ARG $EXPLORER_MODEL_ARG profile fs-explorer stages clarification..planning \
  -agent fs-reporter $PROVIDER_ARG $REPORTER_MODEL_ARG profile fs-reporter stages execution..done \
  $JUDGE_ARG \
