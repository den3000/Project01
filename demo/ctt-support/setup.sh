...
# Формулировки ссылаются на список выполненных вызовов ДОСЛОВНО. Расплывчатое «не
# подтверждённые поиском» судья истолковал как «не подтверждённые пользователем» и
# забраковал перечисление тикетов, которые сам же признал вернувшимися из
# list_user_tickets, — то есть ровно то, что предписывает стадия planning.
...
# Формулировка сужена до наблюдаемого в тексте. «Не обещай сроки, версии и релизы»
# судья растянул на обещание ДЕЙСТВИЯ и забраковал «передал разработке» — ровно то,
# что предписывает `context` эскалации. Секцию `context` судья не видит и такой
# конфликт заметить не может, поэтому его не должно быть в самой формулировке.
"$CLI" -profile support-solve constraints "Не называй конкретных дат, версий и номеров релизов — их нет в контексте."
