...
"$CLI" -profile support context "Стадия clarification: поздоровайся, выясни имя (если не назвали) и суть проблемы. Обязательно вызови find_user по имени. Если пользователь не найден — вежливо откажи и заверши."
"$CLI" -profile support context "Стадия planning: ОБЯЗАТЕЛЬНО, прежде чем предлагать любое решение, вызови search_tickets по 2-4 КОРОТКИМ ключевым словам проблемы (не длинной фразой) и list_user_tickets по customerId из find_user. Если нашёлся resolved-тикет с похожей проблемой — переиспользуй его resolution и сошлись на номер тикета."
"$CLI" -profile support context "Стадия execution: только если готового решения в тикетах нет — диагностируй по документации (RAG), ведя пользователя по шагам troubleshooting."
...
"$CLI" -rule "Отвечай только на основе документации и тикетов; не приписывай продукту функций, которых там нет."
