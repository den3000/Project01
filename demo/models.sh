...
# --- эмбеддер RAG -----------------------------------------------------------------------------
#
# Индексация и запрос обязаны идти ОДНИМ бэкендом: вектора несопоставимы между собой (у gemini
# 3072 измерения, у nomic-embed-text 768), а эмбеддер запроса выбирает не обёртка, а клиент - по
# провайдеру сессии (не-gemini -> ollama). Значит setup обязан индексировать тем же бэкендом.
#
# И под своим ИМЕНЕМ: индексы лежат по имени, поэтому один и тот же `ctt-files` затирался бы при
# каждой смене провайдера, а прогон на другом бэкенде молча сравнивал бы запрос с чужими векторами
# (тихо и правдоподобно - худший вид поломки). С суффиксом два индекса живут рядом, и переключение
# провайдера туда-обратно не требует переиндексации.
embedder_kind() {
  if [ "$PROVIDER" = "ollama" ]; then
    echo "ollama"
  else
...
# Эмбеддинговая модель - отдельный тег, и он НЕ тот, которым разговаривают: клиент зашивает
# `nomic-embed-text`. Без него индексация падает на первом же батче, а запрос - на первом ходу с RAG.
OLLAMA_EMBED_TAG="nomic-embed-text"
...
require_rag_embedder() {
  ...
  require_ollama_up
  case "$(ollama_tags)" in
    *"\"name\":\"$OLLAMA_EMBED_TAG:latest\""* | *"\"name\":\"$OLLAMA_EMBED_TAG\""*) return 0 ;;
  esac
  echo "ОШИБКА: под PROVIDER=ollama RAG эмбедит моделью '$OLLAMA_EMBED_TAG', а она не спулена." >&2
  echo "        Скачать: ollama pull $OLLAMA_EMBED_TAG" >&2
  exit 1
}
