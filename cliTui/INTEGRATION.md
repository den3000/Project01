...
>
> ⚠️ **Экспериментальная реализация TUI в `cliJvmApp` отменена** — она нигде не
> закоммичена и пишется заново. Ниже — **контракт и план**, а НЕ описание дерева.
> Опорная рабочая механика — в песочнице: `ChatViewModel.kt` (UI-agnostic MVI-ядро),
> `TuiChat.kt` (Kotter+Mordant вид), `KotterView.kt` (словарь рендера),
> `PlainChat.kt` (plain-фолбэк).
