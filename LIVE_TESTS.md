...
- **`features:llm`** (нужна локальная Ollama + генеративная модель, по умолчанию `gemma4:26b`;
  ...
  - `LlmWithRagRerankerAnswerLiveTest` — те же 10 вопросов на `BIG_HANDBOOK` двумя путями: plain top-K vs
    ...
    цитата): baseline цитирует **decoy** (grounded ✗), reranked — реальную секцию (grounded ✓). **Только
    Ollama** (бесплатно): числовой скоринг реранкера и JSON grounded-ответа — structured-output, который
    gemini-2.5-flash при зажатых параметрах возвращает пустым, поэтому Gemini-строки тут нет (сравнение
    провайдеров — в `LlmWithRagAnswerLiveTest`).
