...
- **`week_plan`** `{ "from": "YYYY-MM-DD", "to": "YYYY-MM-DD" }` → **план недели как запланированные часы по
  активностям**: суммирует длительность (`dueDate − startDate`) всех запланированных тайм-блоков в диапазоне,
  группируя по названию (`title — Xh Ym (×N)` + итог). Симметрично aTimeLogger `time_by_activity` — для
  сравнения «план vs факт». Полуоткрытый `[from, to)`, зона `WEEK_TZ`. Это основной тул демо (тайм-блоки, а
  не completion).
