...
KMP-модуль (common; таргеты jvm/android/ios). Ядро целиком в `commonMain` (id через
`kotlin.uuid.Uuid`); файловый `JsonFileScheduleStore` (java.io/nio) — только `jvmMain`, его тест —
`jvmTest`. Остальные тесты чистые, в `commonTest` (гоняются на всех платформах).
